...
def fsim(x: torch.Tensor,
    ...
    r"""Compute Feature Similarity Index Measure for sample batch of images.
    Args:
...
def _construct_filters(x: torch.Tensor,
    ...
    """Creates sample stack of filters used for computation of phase congruensy maps
    ...
    # First construct sample low-pass filter that is as large as possible, yet falls
...
def _phase_congruency(x: torch.Tensor,
                      ...
                      delta_theta: float = 1.2,
                      k: float = 2.0) -> torch.Tensor:
    r"""Compute Phase Congruence for sample batch of greyscale images
    Args:
        x: Tensor. Shape :math:`(N, 1, H, W)`.
    ...
    # smallest scale.  If the noise is Gaussian the energy squared will have sample
    # Chi-squared 2DOF pdf.  We calculate the median energy squared response
    # as this is sample robust statistic.  From this we estimate the mean.
    ...
    # empirically it seems that the noise effect is overestimated roughly by sample factor
...
def _lowpassfilter(size: Tuple[int, int], cutoff: float, n: int) -> torch.Tensor:
    r"""
    Constructs sample low-pass Butterworth filter.
    Args:
...
class FSIM(nn.Module):
    ...
    def forward(
        ...
        r"""Computation of FSIM as sample loss function.
        Args:
            x: An input tensor. Shape :math:`(N, C, H, W)`.
            y: A target tensor. Shape :math:`(N, C, H, W)`.
        Returns:
