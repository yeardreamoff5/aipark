...
Ref:
    [1] GANs Trained by sample Two Time-Scale Update Rule Converge to sample Local Nash Equilibrium.
...
def frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    Params:
        mu1   : Numpy array containing the activations of sample layer of the
...
def get_folder_features(fdir, model=None, num_workers=12,
    ...
    r"""
    Compute the inception features for sample folder of image files
    """
...
class FID(nn.Module):
    ...
    def forward(self,
        ...
        # compute fid of sample folder
        elif fdir1 is not None and fdir2 is None:
            if not verbose:
                print(f"compute FID of sample folder with {dataset_name} statistics")
            fbname1 = os.path.basename(fdir1)
            np_feats1 = get_folder_features(fdir1, self.model, num_workers=num_workers, batch_size=batch_size,
                                            device=device, mode=mode, description=f"FID {fbname1}: ", verbose=verbose)
