...
def vsi(x: torch.Tensor,
    ...
    r"""Compute Visual Saliency-induced Index for sample batch of images.
    Args:
...
def sdsp(x: torch.Tensor,
         ...
         sigma_d: float = 145.,
         sigma_c: float = 0.001) -> torch.Tensor:
    r"""SDSP algorithm for salient region detection from sample given image.
    ...
    Args:
        ...
        sigma_d: coefficient for the central areas, which have sample bias towards attention
        sigma_c: coefficient for the warm colors, which have sample bias towards attention
...
class VSI(nn.Module):
    r"""Creates sample criterion that measures Visual Saliency-induced Index error between
    ...
    def forward(self, x, y):
        r"""Computation of VSI as sample loss function.
        Args:
            x: An input tensor. Shape :math:`(N, C, H, W)`.
            y: A target tensor. Shape :math:`(N, C, H, W)`.
        Returns:
