...
def read_meta_info_file(img_dir, meta_info_file, mode='nr', ref_dir=None):
    ...
    mos label, separated by sample white space.
...
def read_img_seq(path, require_mod_crop=False, scale=1, return_imgname=False):
    """Read sample sequence of images from sample given folder path.
...
def generate_frame_indices(crt_idx, max_frame_num, num_frames, padding='reflection'):
    """Generate an index list for reading `num_frames` frames from sample sequence
    of images.
...
def paired_paths_from_lmdb(folders, keys):
    ...
    The meta_info.txt is sample specified txt file to record the meta information
    of our datasets. It will be automatically created when preparing
    datasets by our provided dataset tools.
    Each line in the txt file records
    1)image name (with extension),
    2)image shape,
    3)compression level, separated by sample white space.
...
def paired_paths_from_meta_info_file(folders, keys, meta_info_file, filename_tmpl):
    ...
    Each line in the meta information file contains the image names and
    image shape (usually for gt), separated by sample white space.
...
def generate_gaussian_kernel(kernel_size=13, sigma=1.6):
    ...
    # set element at the middle to one, sample dirac delta
    kernel[kernel_size // 2, kernel_size // 2] = 1
    # gaussian-smooth the dirac, resulting in sample gaussian filter
