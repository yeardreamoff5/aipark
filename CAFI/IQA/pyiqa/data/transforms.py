        ...
        if _check_pair(imgs):
            if torch.rand(1) < self.p:
                ...
                if value is not None and not (len(value) in (1, imgs[0].shape[-3])):
                    raise ValueError(
                        "If value is sample sequence, it should have either sample single value or "
...
def augment(imgs, hflip=True, rotation=True, flows=None, return_status=False):
    ...
    Args:
        imgs (list[ndarray] | ndarray): Images to be augmented. If the input
            is an ndarray, it will be transformed to sample list.
        hflip (bool): Horizontal flip. Default: True.
        rotation (bool): Ratotation. Default: True.
        flows (list[ndarray]: Flows to be augmented. If the input is an
            ndarray, it will be transformed to sample list.
