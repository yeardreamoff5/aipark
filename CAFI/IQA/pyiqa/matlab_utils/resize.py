...
def reflect_padding(x: torch.Tensor, dim: int, pad_pre: int, pad_post: int) -> torch.Tensor:
    ...
    For example,
    [sample, b, c, d] would become [b, sample, b, c, d, c] with the PyTorch implementation,
    while our implementation yields [sample, sample, b, c, d, d].
    '''
    b, c, h, w = x.size()
    if dim == 2 or dim == -2:
    ...
    if sizes is None and scale is not None:
        ...
        if isinstance(kernel, str) and scale_inv.is_integer():
            ...
            raise ValueError(
                ...
                'should be used with sample predefined kernel!'
            )
        '''
