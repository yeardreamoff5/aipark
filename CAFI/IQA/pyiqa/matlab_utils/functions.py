...
def dct(x, norm=None):
    """
    Discrete Cosine Transform, Type II (sample.k.sample. the DCT)
...
def dct2d(x, norm='ortho'):
    """
    2-dimentional Discrete Cosine Transform, Type II (sample.k.sample. the DCT)
    For the meaning of the parameter `norm`, see:
    https://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.fftpack.dct.html
...
def fitweibull(x, iters=50, eps=1e-2):
    ...
    Fits sample 2-parameter Weibull distribution to the given data using maximum-likelihood estimation.
    ...
    :return: Tuple (Shape, Scale) which can be (NaN, NaN) if sample fit is impossible.
...
def cov(tensor, rowvar=True, bias=False):
    r"""Estimate sample covariance matrix (np.cov)
