...
class Extract:
  def __init__(self,serfiq_filter,img_folder_path):
    self.img_folder_path = img_folder_path
    self.serfiq_filter = serfiq_filter
  def landmarks(self):#img_folder_path = "/Users/krc/PycharmProjects/aipark/data/aligned_woman1_img"
    ...
    img_list = self.serfiq_filter["img_path"]
    print("Access Folder: ",img_list[0].replace(os.path.basename(img_list[0]),""))
    ...
    for file in img_list:  # default: img_list
      ...
      # create to save path for landmarks
      pickle_path = file.replace(".jpg", ".txt")

      # create to save path for masked image
  ...
  def degree(self):
    ...
    for img, txt in zip(img_list, txt_list):  # default: (img_list,txt_list)
      ...
      degree = [img, mask_path, rotate_degree[0], rotate_degree[1], rotate_degree[2]]
      total_degree.append(degree)
      print(f"{img}'s Roll,Pitch,Yaw")
      print(degree)
    pd_degree = pd.DataFrame(total_degree, columns=["img_path", "masked_path", "roll", "pitch", "yaw"])
    pd_degree.to_csv(self.img_folder_path + "/Degree.csv")
    print("Complete to save Degree to csv")
    detect_filter = pd_degree.dropna()
    return detect_filter